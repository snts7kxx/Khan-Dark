...
async function showSplashScreen() {
  ...
  splashScreen.innerHTML = '<span style="color:white;"<strong>KHAN<strong></span><span style="color:#af00ff;"<strong>DESTROYER<strong></span>';
    ...
    if (body?.includes('"operationName":"updateUserVideoProgress"')) {
      try {
        ...
        if (bodyObj.variables?.input) {
          ...
          sendToast("🔄 | Vídeo concluído!", 2000);
        }
      } catch (e) {}
    }
    ...
    while (window.khandarkDominates) {
      for (const selector of selectors) {
        ...
        if (element?.innerText === "Mostrar resumo") {
          sendToast("🎉 | Questão concluído!", 3000);
        }
      }
      await delay(3000); 
    }
