...
document.head.appendChild(Object.assign(document.createElement('style'), {
    innerHTML: `
        ...
        .kd-splash-screen {
            ...
            user-select: none; font-family: MuseoSans, sans-serif;
        }
        .kd-particles { position: absolute; width: 100%; height: 100%; overflow: hidden; }
        .kd-particle { position: absolute; background: #af00ff; border-radius: 50%; animation: float 15s infinite ease-in-out; opacity: 0.1; }
        .kd-splash-content { position: relative; z-index: 2; text-align: center; animation: slideUp 0.8s cubic-bezier(0.22, 1, 0.36, 1); }
        .kd-logo-container { position: relative; margin-bottom: 40px; }
        .kd-logo-text { font-size: 72px; font-weight: bold; letter-spacing: 4px; position: relative; display: inline-block; }
        .kd-logo-khan { color: white; text-shadow: 0 0 20px rgba(255, 255, 255, 0.5); }
        ...
        .kd-loader-container { margin: 40px 0; }
        ...
        .kd-progress-bar { width: 100%; height: 6px; background: rgba(175, 0, 255, 0.1); border-radius: 10px; overflow: hidden; position: relative; box-shadow: inset 0 0 10px rgba(0, 0, 0, 0.5); }
        .kd-progress-fill { height: 100%; background: linear-gradient(90deg, #8b00cc, #af00ff, #d966ff, #af00ff); background-size: 200% 100%; width: 0%; transition: width 0.5s cubic-bezier(0.22, 1, 0.36, 1); box-shadow: 0 0 20px #af00ff; animation: shimmer 2s linear infinite; position: relative; }
        .kd-progress-fill::after { content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.3), transparent); animation: slide 1.5s infinite; }
        .kd-progress-percent { text-align: center; color: rgba(255, 255, 255, 0.7); font-size: 14px; margin-top: 10px; font-weight: 500; }
        .kd-plugin-status { color: rgba(255, 255, 255, 0.5); font-size: 13px; margin-top: 20px; min-height: 20px; letter-spacing: 1px; }
        .kd-version { position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); color: rgba(175, 0, 255, 0.5); font-size: 12px; letter-spacing: 2px; }
    `
}));
...
function setupMain(){
    ...
    /* =============================================
       INTERCEPTOR ÚNICO — todos os módulos aqui
       ============================================= */
    ...
    window.fetch = async function(input, init = {}) {
        ...
        if (url.includes('getAssessmentItem') && body) {
            ...
            try {
                ...
                // ✅ CORREÇÃO PRINCIPAL: condição removida — modifica sempre que houver conteúdo
    ...
    (function () {
        ...
        let khanDarkDominates = true; // ✅ CORREÇÃO: declarado com let (sem poluir escopo global)
